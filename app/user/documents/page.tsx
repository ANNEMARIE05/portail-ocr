...
export default function PageDocuments() {
  ...
  const gererDrop = useCallback((e: React.DragEvent) => {
    ...
    ajouterFichiers(fichiersDeposes)
  }, [])

  const gererSelectionFichier = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      const fichiersList = Array.from(e.target.files)
      ajouterFichiers(fichiersList)
    }
  }

  const ajouterFichiers = async (nouveauxFichiers: File[]) => {
    for (const fichier of nouveauxFichiers) {
      ...
      for (let i = 0; i <= 100; i += 20) {
        ...
        setFichiers(prev => prev.map(f => 
          f.fichier === fichier ? { ...f, progression: i } : f
        ))
      }
      ...
      try {
        ...
        if (reponse.succes && reponse.donnees) {
          setFichiers(prev => prev.map(f => 
            f.fichier === fichier ? { ...f, statut: 'termine', resultat: reponse.donnees, progression: 100 } : f
          ))
        } else {
          setFichiers(prev => prev.map(f => 
        ...
        setFichiers(prev => prev.map(f => 
          f.fichier === fichier ? { ...f, statut: 'erreur', erreur: 'Erreur lors du traitement' } : f
        ))
      }
    }
  }
    ...
    <div className="space-y-6">
      ...
      >
        <CardContent className="flex flex-col items-center justify-center p-12 text-center">
          <div className={cn(
            ...
            estSurvol 
              ? "bg-slate-900 shadow-xl shadow-slate-900/30 scale-110" 
              : "bg-slate-100"
          )}>
            <Upload className={cn(
              "h-8 w-8 transition-colors",
              estSurvol ? "text-white" : "text-slate-500"
            )} />
          </div>
          ...
          <div className="flex items-center gap-4">
            <label>
              <input
                ...
                className="sr-only"
              />
              <Button asChild className="gap-2 bg-slate-900 hover:bg-slate-800 text-white cursor-pointer">
                <span>
      ...
      {fichiers.length > 0 && (
        <Card className="border-slate-200/60">
          <CardHeader className="pb-3">
            ...
            <CardDescription>{fichiers.length} fichier{fichiers.length > 1 ? 's' : ''}</CardDescription>
          </CardHeader>
      ...
      <Card className="border-slate-200/60 bg-slate-50/50">
        <CardContent className="p-6">
          <h4 className="text-sm font-semibold text-slate-900 mb-3">Conseils pour de meilleurs resultats</h4>
          <ul className="space-y-2 text-sm text-slate-600">
            <li className="flex items-start gap-2">
              <Check className="h-4 w-4 text-emerald-600 mt-0.5 shrink-0" />
              <span>Utilisez des images de haute qualite (300 DPI minimum)</span>
            </li>
            <li className="flex items-start gap-2">
              <Check className="h-4 w-4 text-emerald-600 mt-0.5 shrink-0" />
              <span>Assurez-vous que le texte est bien lisible et non flou</span>
            </li>
            <li className="flex items-start gap-2">
              <Check className="h-4 w-4 text-emerald-600 mt-0.5 shrink-0" />
              <span>Evitez les documents avec trop de bruit ou d&apos;arriere-plans complexes</span>
            </li>
            <li className="flex items-start gap-2">
              <Check className="h-4 w-4 text-emerald-600 mt-0.5 shrink-0" />
              <span>Les PDFs numeriques donnent de meilleurs resultats que les scans</span>
            </li>
          </ul>
        </CardContent>
      </Card>
